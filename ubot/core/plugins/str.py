...
async def ping_cmd(client, message):
    ...
    time = await get_var(client.me.id, "emoji2")
    cos_time2 = time if pong else TIME
    time_id = await get_var(client.me.id, "emoji_id2")
    cos_time = time_id if time_id else "5974513952158128296"
    
    if gua == True:
      _ping = f"""
**<emoji id={cos_ping}>{cos_ping2}</emoji> Pong !!**
**<emoji id={cos_pong}>{cos_pong2}</emoji>`{str(delta_ping).replace('.', ',')}ms`**
 **<emoji id={cos_time}>{cos_time2}</emoji><b>Uptime Ubot :</b> <code>{uptime}</code>**"""
    elif gua == False:
      _ping = f"""
**{cos_ping2} Pong !!**
